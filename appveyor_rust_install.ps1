...
# This installs the version of Rust specified by the "channel" and "target"
# environment variables from the build matrix. By default, Rust will be installed
# to C:\Rust for easy usage, but this path can be overridden by setting the
# RUST_INSTALL_DIR environment variable. The URL to download rust distributions
# defaults to https://static.rust-lang.org/dist/ but can overridden by setting
# the RUST_DOWNLOAD_URL environment variable.
#
